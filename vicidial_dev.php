...
if ($campaign_login_list > 0)
	...
	while ($camps_to_print > $o) 
		...
		if ($VD_campaign)
			{
			if ( (preg_match("$VD_campaign",$rowx[0])) and (strlen($VD_campaign) == strlen($rowx[0])) )
...
echo "<!-- ILPV -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc_ru/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_ru/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Russian <img src=\"../agc_ru/images/ru.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "</TR></TABLE>\n";
...
echo "<TD ALIGN=LEFT VALIGN=BOTTOM><IMG SRC=\"../agc_ru/images/vdc_tab_vicidial.gif\" Border=0></TD>";
...
$VDloginDISPLAY=0;
	...
	if($auth>0)
		...
		$LOGgroup_shiftsSQL = preg_replace('  ','',$row[2]);
		$LOGgroup_shiftsSQL = preg_replace(' ',"','",$LOGgroup_shiftsSQL);
		$LOGgroup_shiftsSQL = "shift_id IN('$LOGgroup_shiftsSQL')";
		$agent_status_viewable_groups = $row[3];
		$agent_status_viewable_groupsSQL = preg_replace('  ','',$agent_status_viewable_groups);
		$agent_status_viewable_groupsSQL = preg_replace(' ',"','",$agent_status_viewable_groupsSQL);
		...
		if ( ( (preg_match("START|ALL",$shift_enforcement)) and (!preg_match("OFF",$VU_agent_shift_enforcement_override)) ) or (preg_match("START|ALL",$VU_agent_shift_enforcement_override)) )
			...
			if ( (strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1) )
				...
				while ( ($shifts_to_print > $o) and ($shift_ok < 1) )
					...
					if (preg_match("$wday",$shift_weekdays))
						{
		...
		if ( (!preg_match(" $VD_campaign ",$LOGallowed_campaigns)) and (!preg_match("ALL-CAMPAIGNS",$LOGallowed_campaigns)) )
			{
			echo "<title>Agent web client: Логин Кампании</title>\n";
			echo "</head>\n";
			echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0 onResize=\"browser_dimensions();\"  onLoad=\"browser_dimensions();\">\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc_ru/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_ru/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Russian <img src=\"../agc_ru/images/ru.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";			echo "</TR></TABLE>\n";
		...
		if($CAMPactive>0)
			...
			while ($w < $HK_statuses_camp)
				...
				if ($w < 3)
					{$HKboxA = "$HKboxA <div class=\"skb_text\">$HKhotkey[$w]</div> - $HKstatus[$w] - $HKstatus_name[$w]<BR>";}
				if ( ($w >= 3) and ($w < 6) )
					{$HKboxB = "$HKboxB <div class=\"skb_text\">$HKhotkey[$w]</div> - $HKstatus[$w] - $HKstatus_name[$w]<BR>";}
				if ($w >= 6)
					{$HKboxC = "$HKboxC <div class=\"skb_text\">$HKhotkey[$w]</div> - $HKstatus[$w] - $HKstatus_name[$w]<BR>";}
				$w++;
				}
...
### logged in and use that phone login account
	...
	$session_ext = preg_replace("[^a-z0-9]", "", $extension);
	...
	#	echo "<B>You have logged in as user: $VD_login на phone: $SIP_user в кампанию: $VD_campaign</B><BR>\n";
		...
		if ($auto_dial_level > 0)
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$linkB,$mel,$stmt,'01046',$VD_login,$server_ip,$session_name,$one_mysql_log);}
				...
				mysqli_close($linkB);
				mysqli_select_db("$VARDB_database", $link);
				}
...
?>
	...
	var DiaLControl_auto_HTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_pause_OFF_ru.gif\" border=0 alt=\" Пауза \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc_ru/images/vdc_LB_resume.gif\" border=0 alt=\"Резюме\"></a>";
	var DiaLControl_auto_HTML_ready = "<a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADpause');\"><IMG SRC=\"../agc_ru/images/vdc_LB_pause_ru.gif\" border=0 alt=\" Пауза \"></a><IMG SRC=\"../agc_ru/images/vdc_LB_resume_OFF_ru.gif\" border=0 alt=\"Резюме\">";
	var DiaLControl_auto_HTML_OFF = "<IMG SRC=\"../agc_ru/images/vdc_LB_pause_OFF_ru.gif\" border=0 alt=\" Пауза \"><IMG SRC=\"../agc_ru/images/vdc_LB_resume_OFF_ru.gif\" border=0 alt=\"Резюме\">";
	var DiaLControl_manual_HTML = "<a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc_ru/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Набрать следующий номер\"></a>";
	var image_blank = new Image();
		image_blank.src="../agc_ru/images/blank.gif";
	var image_livecall_OFF = new Image();
		image_livecall_OFF.src="../agc_ru/images/agc_live_call_OFF.gif";
	var image_livecall_ON = new Image();
		image_livecall_ON.src="../agc_ru/images/agc_live_call_ON.gif";
	var image_livecall_DEAD = new Image();
		image_livecall_DEAD.src="../agc_ru/images/agc_live_call_DEAD.gif";
	var image_LB_dialnextnumber = new Image();
		image_LB_dialnextnumber.src="../agc_ru/images/vdc_LB_dialnextnumber.gif";
	var image_LB_hangupcustomer = new Image();
		image_LB_hangupcustomer.src="../agc_ru/images/vdc_LB_hangupcustomer_ru.gif";
	var image_LB_transferconf = new Image();
		image_LB_transferconf.src="../agc_ru/images/vdc_LB_transferconf.gif";
	var image_LB_grabparkedcall = new Image();
		image_LB_grabparkedcall.src="../agc_ru/images/vdc_LB_grabparkedcall.gif";
	var image_LB_parkcall = new Image();
		image_LB_parkcall.src="../agc_ru/images/vdc_LB_parkcall.gif";
	var image_LB_webform = new Image();
		image_LB_webform.src="../agc_ru/images/vdc_LB_webform.gif";
	var image_LB_stoprecording = new Image();
		image_LB_stoprecording.src="../agc_ru/images/vdc_LB_stoprecording_ru.gif";
	var image_LB_startrecording = new Image();
		image_LB_startrecording.src="../agc_ru/images/vdc_LB_startrecording_ru.gif";
	var image_LB_pause = new Image();
		image_LB_pause.src="../agc_ru/images/vdc_LB_pause_ru.gif";
	var image_LB_resume = new Image();
		image_LB_resume.src="../agc_ru/images/vdc_LB_resume.gif";
	var image_LB_senddtmf = new Image();
		image_LB_senddtmf.src="../agc_ru/images/vdc_LB_senddtmf.gif";
	var image_LB_dialnextnumber_OFF = new Image();
		image_LB_dialnextnumber_OFF.src="../agc_ru/images/vdc_LB_dialnextnumber_OFF.gif";
	var image_LB_hangupcustomer_OFF = new Image();
		image_LB_hangupcustomer_OFF.src="../agc_ru/images/vdc_LB_hangupcustomer_OFF_ru.gif";
	var image_LB_transferconf_OFF = new Image();
		image_LB_transferconf_OFF.src="../agc_ru/images/vdc_LB_transferconf_OFF_ru.gif";
	var image_LB_grabparkedcall_OFF = new Image();
		image_LB_grabparkedcall_OFF.src="../agc_ru/images/vdc_LB_grabparkedcall_OFF.gif";
	var image_LB_parkcall_OFF = new Image();
		image_LB_parkcall_OFF.src="../agc_ru/images/vdc_LB_parkcall_OFF_ru.gif";
	var image_LB_webform_OFF = new Image();
		image_LB_webform_OFF.src="../agc_ru/images/vdc_LB_webform_OFF.gif";
	var image_LB_stoprecording_OFF = new Image();
		image_LB_stoprecording_OFF.src="../agc_ru/images/vdc_LB_stoprecording_OFF.gif";
	var image_LB_startrecording_OFF = new Image();
		image_LB_startrecording_OFF.src="../agc_ru/images/vdc_LB_startrecording_OFF.gif";
	var image_LB_pause_OFF = new Image();
		image_LB_pause_OFF.src="../agc_ru/images/vdc_LB_pause_OFF_ru.gif";
	var image_LB_resume_OFF = new Image();
		image_LB_resume_OFF.src="../agc_ru/images/vdc_LB_resume_OFF_ru.gif";
	var image_LB_senddtmf_OFF = new Image();
		image_LB_senddtmf_OFF.src="../agc_ru/images/vdc_LB_senddtmf_OFF.gif";
	...
	function volume_control(taskdirection,taskvolchannel,taskagentmute) 
		...
		if (taskagentmute=='AgenT')
			...
			if (taskdirection=='MUTING')
				{
				document.getElementById("AgentMuteSpan").innerHTML = "<a href=\"#CHAN-" + agentchannel + "\" onclick=\"volume_control('UNMUTE','" + agentchannel + "','AgenT');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_UNMUTE.gif\" Border=0></a>";
				}
			else
				{
				document.getElementById("AgentMuteSpan").innerHTML = "<a href=\"#CHAN-" + agentchannel + "\" onclick=\"volume_control('MUTING','" + agentchannel + "','AgenT');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_MUTE.gif\" Border=0></a>";
				}
			}
...
// filter manual dialstring and pass на to originate call
	function SendManualDial(taskFromConf)
		...
		if (taskFromConf == 'YES')
			...
			document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc_ru/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Набор номера С Клиентом\" style=\"vertical-align:middle\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc_ru/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Парковать Набор номера Клиента\" style=\"vertical-align:middle\"></a>";
	...
	function basic_originate_call(tasknum,taskprefix,taskreverse,taskdialvalue,tasknowait,taskconfxfer,taskcid,taskusegroupalias) 
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//		document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc_ru/images/vdc_XB_hangupxferline.gif\" border=0 alt=\"Разъединение Xfer Линии\"></a>";
						}
					}
				}
			delete xmlhttp;
									...
									if ( (HidEMonitoRSessionS==1) && (channelfieldA.match(/ASTblind/)) )
										...
										if (channelfieldA.match(regRNnolink))
											...
											if (volumecontrol_active!=1)
												...
												{
												live_conf_HTML = live_conf_HTML + "<tr bgcolor=\"" + row_color + "\"><td><font class=\"log_text\">" + loop_ct + "</td><td><font class=\"" + chan_name_color + "\">" + channelfieldA + "</td><td><font class=\"log_text\"><a href=\"#\" onclick=\"livehangup_send_hangup('" + channelfieldA + "');return false;\">РАЗЪЕДИНЕНИЕ</a></td><td><a href=\"#\" onclick=\"volume_control('UP','" + channelfieldA + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_up.gif\" Border=0></a> &nbsp; <a href=\"#\" onclick=\"volume_control('DOWN','" + channelfieldA + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_down.gif\" Border=0></a> &nbsp; &nbsp; &nbsp; <a href=\"#\" onclick=\"volume_control('MUTING','" + channelfieldA + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_MUTE.gif\" Border=0></a> &nbsp; <a href=\"#\" onclick=\"volume_control('UNMUTE','" + channelfieldA + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_UNMUTE.gif\" Border=0></a></td></tr>";
												}
											}
										}
	...
	function conf_send_recording(taskconfrectype,taskconfrec,taskconffile) 
		...
		if (xmlhttp) 
			...
			//	filename = filedate + "_" + user_abb;
				...
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('StopMonitorConf','" + taskconfrec + "','" + filename + "');return false;\"><IMG SRC=\"../agc_ru/images/vdc_LB_stoprecording_ru.gif\" border=0 alt=\"Остановить Запись\"></a>";

				if (LIVE_campaign_recording == 'ALLFORCE')
					{
					document.getElementById("RecorDControl").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_startrecording_OFF.gif\" border=0 alt=\"Начать Запись\">";
					}
				else
					{
			...
			if (taskconfrectype == 'StopMonitorConf')
				...
				var channelrec = "Local/" + conf_silent_prefix + '' + taskconfrec + "@" + ext_context;
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + taskconfrec + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_LB_startrecording_ru.gif\" border=0 alt=\"Начать Запись\"></a>";
				if (LIVE_campaign_recording == 'ALLFORCE')
					{
					document.getElementById("RecorDControl").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_startrecording_OFF.gif\" border=0 alt=\"Начать Запись\">";
					}
	...
	//	conf_dialed=1;
		...
		if (xmlhttpXF) 
			...
			if (taskvar == 'ParK')
				...
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('FROMParK','" + redirectdestination + "','" + redirectdestserverip + "');return false;\"><IMG SRC=\"../agc_ru/images/vdc_LB_grabparkedcall.gif\" border=0 alt=\"Захват Ожидающего Вызова\"></a>";
				customerparked=1;
				}
			if (taskvar == 'FROMParK')
				{
				...
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + redirectdestination + "','" + redirectdestserverip + "');return false;\"><IMG SRC=\"../agc_ru/images/vdc_LB_parkcall.gif\" border=0 alt=\"Парковать Вызов\"></a>";
				customerparked=0;
				}
			...
			//		alert(xmlhttp.responseText);
				...
				//		alert("VICIDIAL Call log entered:\n" + document.vicidial_form.uniqueid.value);
						if ( (taskMDstage != "start") && (VDstop_rec_after_each_call == 1) )
							{
							var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + session_id + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_LB_startrecording_ru.gif\" border=0 alt=\"Начать Запись\"></a>";
					...
					//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"leave_3way_call('FIRST');return false;\"><IMG SRC=\"../agc_ru/images/vdc_XB_leave3waycall.gif\" border=0 alt=\"ОСТАВИТЬ 3-WAY ЗАПРОС\" style=\"vertical-align:middle\"></a>";

								document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc_ru/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Набор номера С Клиентом\" style=\"vertical-align:middle\">";

								document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc_ru/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Парковать Набор номера Клиента\" style=\"vertical-align:middle\">";

								document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc_ru/images/vdc_XB_hangupxferline.gif\" border=0 alt=\"Разъединение Xfer Линии\" style=\"vertical-align:middle\"></a>";

								document.getElementById("HangupBothLines").innerHTML ="<a href=\"#\" onclick=\"bothcall_send_hangup();return false;\"><IMG SRC=\"../agc_ru/images/vdc_XB_hangupbothlines.gif\" border=0 alt=\"Разъединить Обе Линии\" style=\"vertical-align:middle\"></a>";
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc_ru/images/vdc_LB_parkcall.gif\" border=0 alt=\"Парковать Вызов\"></a>";

								document.getElementById("HangupControl").innerHTML = "<a href=\"#\" onclick=\"dialedcall_send_hangup();\"><IMG SRC=\"../agc_ru/images/vdc_LB_hangupcustomer_ru.gif\" border=0 alt=\"Разъединить Клиента\"></a>";

								document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('ON');\"><IMG SRC=\"../agc_ru/images/vdc_LB_transferconf.gif\" border=0 alt=\"Перевод - Конференция\"></a>";

								document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc_ru/images/vdc_XB_localcloser.gif\" border=0 alt=\"ЛОКАЛЬНЫЙ CLOSER\" style=\"vertical-align:middle\"></a>";

								document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc_ru/images/vdc_XB_blindtransfer.gif\" border=0 alt=\"Слепой Перевод Набора номера\" style=\"vertical-align:middle\"></a>";

								document.getElementById("DialBlindVMail").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRVMAIL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc_ru/images/vdc_XB_ammessage.gif\" border=0 alt=\"Blind Transfer VMail Message\" style=\"vertical-align:middle\"></a>";

								document.getElementById("VolumeUpSpan").innerHTML = "<a href=\"#\" onclick=\"volume_control('UP','" + MDchannel + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_up.gif\" Border=0></a>";
								document.getElementById("VolumeDownSpan").innerHTML = "<a href=\"#\" onclick=\"volume_control('DOWN','" + MDchannel + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_down.gif\" Border=0></a>";

								if (quick_transfer_button == 'IN_GROUP')
									{
									document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc_ru/images/vdc_LB_quickxfer.gif\" border=0 alt=\"Быстрой передачи\"></a>";
									}
								...
								if ( (quick_transfer_button == 'PRESET_1') || (quick_transfer_button == 'PRESET_2') || (quick_transfer_button == 'PRESET_3') || (quick_transfer_button == 'PRESET_4') || (quick_transfer_button == 'PRESET_5') )
									{
									document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc_ru/images/vdc_LB_quickxfer.gif\" border=0 alt=\"Быстрой передачи\"></a>";
								...
								if (call_requeue_button > 0)
									...
									if ( (CloserSelectChoices.match(regCRB)) || (VU_closer_campaigns.match(regCRB)) )
										{
										document.getElementById("ReQueueCall").innerHTML =  "<a href=\"#\" onclick=\"call_requeue_launch();return false;\"><IMG SRC=\"../agc_ru/images/vdc_LB_requeue_call.gif\" border=0 alt=\"Re-Queue Call\"></a>";
										}
									else
										{
										document.getElementById("ReQueueCall").innerHTML =  "<IMG SRC=\"../agc_ru/images/vdc_LB_requeue_call_OFF.gif\" border=0 alt=\"Re-Queue Call\">";
										}
									}
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (UDresponse_status == 'GOOD')
						...
						document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormRefresH();\"><IMG SRC=\"../agc_ru/images/vdc_LB_webform.gif\" border=0 alt=\"Web Форма\"></a>\n";
						if (enable_second_webform > 0)
							{
							document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormTwoRefresH();\"><IMG SRC=\"../agc_ru/images/vdc_LB_webform_two.gif\" border=0 alt=\"Web Форма 2\"></a>\n";
							}
						}
					else
						{
...
// Send the Manual Набрать следующий номер request
	function ManualDialNext(mdnCBid,mdnBDleadid,mdnDiaLCodE,mdnPhonENumbeR,mdnStagE,mdVendorid,mdgroupalias)
		...
		if (dial_method == "INBOUND_MAN")
			...
			document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_pause_OFF_ru.gif\" border=0 alt=\" Пауза \"><IMG SRC=\"../agc_ru/images/vdc_LB_resume_OFF_ru.gif\" border=0 alt=\"Резюме\"><BR><IMG SRC=\"../agc_ru/images/vdc_LB_dialnextnumber_OFF.gif\" border=0 alt=\"Набрать следующий номер\">";
			}
		else
			{
			document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_dialnextnumber_OFF.gif\" border=0 alt=\"Набрать следующий номер\">";
			}
			...
			//		alert(xmlhttp.responseText);
					...
					if ( (MDnextCID.match(regMNCvar)) || (MDnextCID.match(regMDFvarDNC)) || (MDnextCID.match(regMDFvarCAMP)) )
						...
						if (starting_dial_level == 0)
							{
							document.getElementById("DiaLControl").innerHTML = "<a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc_ru/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Набрать следующий номер\"></a>";
							}
						else
							{
							if (dial_method == "INBOUND_MAN")
								...
								document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_pause_OFF_ru.gif\" border=0 alt=\" Пауза \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc_ru/images/vdc_LB_resume.gif\" border=0 alt=\"Резюме\"></a><BR><a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc_ru/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Набрать следующий номер\"></a>";
								}
							else
								{
	...
	function AutoDial_ReSume_PauSe(taskaction,taskagentlog,taskwrapup,taskstatuschange,temp_reason)
		...
		if (taskaction == 'VDADready')
			...
			if (dial_method == "INBOUND_MAN")
				...
				document.getElementById("DiaLControl").innerHTML = "<a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADpause');\"><IMG SRC=\"../agc_ru/images/vdc_LB_pause_ru.gif\" border=0 alt=\" Пауза \"></a><IMG SRC=\"../agc_ru/images/vdc_LB_resume_OFF_ru.gif\" border=0 alt=\"Резюме\"></a><BR><a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc_ru/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Набрать следующий номер\"></a>";
				}
			else
				{
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							if (lastcustserverip == server_ip)
								{
								document.getElementById("VolumeUpSpan").innerHTML = "<a href=\"#\" onclick=\"volume_control('UP','" + lastcustchannel + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_up.gif\" Border=0></a>";
								document.getElementById("VolumeDownSpan").innerHTML = "<a href=\"#\" onclick=\"volume_control('DOWN','" + lastcustchannel + "','');return false;\"><IMG SRC=\"../agc_ru/images/vdc_volume_down.gif\" Border=0></a>";
								}
...
// refresh the content of the web form URL
	function WebFormRefresH(taskrefresh,submittask,force_webvars_refresh) 
		...
		if (taskrefresh == 'OUT')
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormRefresH('IN');\"><IMG SRC=\"../agc_ru/images/vdc_LB_webform.gif\" border=0 alt=\"Web Форма\"></a>\n";
			}
		else 
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOut=\"WebFormRefresH('OUT');\"><IMG SRC=\"../agc_ru/images/vdc_LB_webform.gif\" border=0 alt=\"Web Форма\"></a>\n";
			}
		}
...
// refresh the content of the second web form URL
	function WebFormTwoRefresH(taskrefresh,submittask) 
		...
		if (enable_second_webform > 0)
			...
			if (taskrefresh == 'OUT')
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormTwoRefresH('IN');\"><IMG SRC=\"../agc_ru/images/vdc_LB_webform_two.gif\" border=0 alt=\"Web Форма 2\"></a>\n";
				}
			else 
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOut=\"WebFormTwoRefresH('OUT');\"><IMG SRC=\"../agc_ru/images/vdc_LB_webform_two.gif\" border=0 alt=\"Web Форма 2\"></a>\n";
				}
			}
		}
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo,nodeletevdac) 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			if( document.images ) { document.images['livecall'].src = image_livecall_OFF.src;}
			document.getElementById("WebFormSpan").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_webform_OFF.gif\" border=0 alt=\"Web Форма\">";
			if (enable_second_webform > 0)
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_webform_two_OFF.gif\" border=0 alt=\"Web Форма 2\">";
				}
			document.getElementById("ParkControl").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_parkcall_OFF_ru.gif\" border=0 alt=\"Парковать Вызов\">";
			document.getElementById("HangupControl").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_hangupcustomer_OFF_ru.gif\" border=0 alt=\"Разъединить Клиента\">";
			document.getElementById("XferControl").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_transferconf_OFF_ru.gif\" border=0 alt=\"Перевод - Конференция\">";
			document.getElementById("LocalCloser").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_XB_localcloser_OFF.gif\" border=0 alt=\"ЛОКАЛЬНЫЙ CLOSER\" style=\"vertical-align:middle\">";
			document.getElementById("DialBlindTransfer").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_XB_blindtransfer_OFF.gif\" border=0 alt=\"Слепой Перевод Набора номера\" style=\"vertical-align:middle\">";
			document.getElementById("DialBlindVMail").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_XB_ammessage_OFF.gif\" border=0 alt=\"Blind Transfer VMail Message\" style=\"vertical-align:middle\">";
			document.getElementById("VolumeUpSpan").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_volume_up_off.gif\" Border=0>";
			document.getElementById("VolumeDownSpan").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_volume_down_off.gif\" Border=0>";

			if (quick_transfer_button_enabled > 0)
				{document.getElementById("QuickXfer").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_quickxfer_OFF.gif\" border=0 alt=\"Быстрой передачи\">";}
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() 
		...
		//	document.getElementById("Leave3WayCall").innerHTML ="<IMG SRC=\"../agc_ru/images/vdc_XB_leave3waycall_OFF.gif\" border=0 alt=\"ОСТАВИТЬ 3-WAY ЗАПРОС\">";

			document.getElementById("DialWithCustomer").innerHTML ="<a href=\"#\" onclick=\"SendManualDial('YES');return false;\"><IMG SRC=\"../agc_ru/images/vdc_XB_dialwithcustomer.gif\" border=0 alt=\"Набор номера С Клиентом\" style=\"vertical-align:middle\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<a href=\"#\" onclick=\"xfer_park_dial();return false;\"><IMG SRC=\"../agc_ru/images/vdc_XB_parkcustomerdial.gif\" border=0 alt=\"Парковать Набор номера Клиента\" style=\"vertical-align:middle\"></a>";

			document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc_ru/images/vdc_XB_hangupxferline_OFF.gif\" border=0 alt=\"Разъединение Xfer Линии\" style=\"vertical-align:middle\">";
...
// GLOBAL FUNCTIONS
	...
	function HotKeys(HKstate) 
		...
		if ( (HKstate == 'ON') && (HKbutton_allowed == 1) )
			...
			showDiv('HotKeyEntriesBox');
			hot_keys_active = 1;
			document.getElementById("hotkeysdisplay").innerHTML = "<a href=\"#\" onMouseOut=\"HotKeys('OFF')\"><IMG SRC=\"../agc_ru/images/vdc_XB_hotkeysactive.gif\" border=0 alt=\"АКТИВНЫЕ ГОРЯЧИЕ КЛАВИШИ\"></a>";
			}
		else
			{
			hideDiv('HotKeyEntriesBox');
			hot_keys_active = 0;
			document.getElementById("hotkeysdisplay").innerHTML = "<a href=\"#\" onMouseOver=\"HotKeys('ON')\"><IMG SRC=\"../agc_ru/images/vdc_XB_hotkeysactive_OFF.gif\" border=0 alt=\"НЕАКТИВНЫЕ ГОРЯЧИЕ КЛАВИШИ\"></a>";
			}
		}
	...
	function ShoWTransferMain(showxfervar,showoffvar)
		...
		if (VU_vicidial_transfers == '1')
			...
			if (showxfervar == 'ON')
				...
				document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('OFF','YES');\"><IMG SRC=\"../agc_ru/images/vdc_LB_transferconf.gif\" border=0 alt=\"Перевод - Конференция\"></a>";
				if (quick_transfer_button_enabled > 0)
					{document.getElementById("QuickXfer").innerHTML = "<IMG SRC=\"../agc_ru/images/vdc_LB_quickxfer_OFF.gif\" border=0 alt=\"Быстрой передачи\">";}
				}
			else
...
<TD ALIGN=LEFT WIDTH=115><A HREF="#" onclick="MainPanelToFront('NO');"><IMG SRC="../agc_ru/images/vdc_tab_vicidial.gif" ALT="MAIN" WIDTH=115 HEIGHT=30 Border=0></A></TD>
<TD ALIGN=LEFT WIDTH=105><A HREF="#" onclick="ScriptPanelToFront();"><IMG SRC="../agc_ru/images/vdc_tab_script.gif" ALT="SCRIPT" WIDTH=105 HEIGHT=30 Border=0></A></TD>
<TD WIDTH=<?php echo $HSwidth ?> VALIGN=MIDDLE ALIGN=CENTER><div class="body_text">&nbsp; <span id=status>LIVE</span>&nbsp; &nbsp;session ID: <span id=sessionIDspan></span>&nbsp; &nbsp;<span id=AgentСтатусCalls></span></TD>
<TD WIDTH=109><IMG SRC="../agc_ru/images/agc_live_call_OFF.gif" NAME=livecall ALT="Живой Вызов" WIDTH=109 HEIGHT=30 Border=0></TD>
</TR></TABLE>
</span>
...
<span style="position:absolute;left:300px;top:<?php echo $MBheight ?>px;z-index:12;" id="ManuaLDiaLButtons"><div class="body_text">
...
<span style="position:absolute;left:300px;top:<?php echo $CBheight ?>px;z-index:13;" id="CallbacksButtons"><div class="body_text">
<span id="CBstatusSpan">X АКТИВНЫЕ ВЫЗОВBACK</span> <BR>
</div></span>

<span style="position:absolute;left:500px;top:<?php echo $CBheight ?>px;z-index:14;" id="PauseCodeButtons"><div class="body_text">
<span id="PauseCodeLinkSpan"></span> <BR>
</div></span>

<span style="position:absolute;left:<?php echo $SCwidth ?>px;top:49px;z-index:18;" id="SecondSspan"><div class="body_text"> секунды:
<span id="SecondSDISP"> &nbsp; &nbsp; </span>
</div></span>

<span style="position:absolute;left:<?php echo $AMwidth ?>px;top:<?php echo $AMheight ?>px;z-index:22;" id="AgentMuteANDPreseTDiaL"><div class="body_text">
	<?php
	if ($PreseT_DiaL_LinKs)
		{
		echo "<a href=\"#\" onclick=\"DtMf_PreSet_a_DiaL();return false;\"><div class=\"body_tiny\">D1 - НАБОР НОМЕРА</div></a>\n";
		echo "<BR>\n";
		echo "<a href=\"#\" onclick=\"DtMf_PreSet_b_DiaL();return false;\"><div class=\"body_tiny\">D2 - НАБОР НОМЕРА</div></a>\n";
		}
	else {echo "<BR>\n";}
	?>
<BR><BR> &nbsp; <BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:52;" id="NeWManuaLDiaLBox">
	...
	<table><tr>
	<td align=right><div class="body_text"> Код страны: </td>
	<td align=left><div class="body_text"><input type=text size=7 maxlength=10 name=MDDiaLCodE class="cust_form" value="1">&nbsp; (Обычно 7 для России и некоторых стран СНГ)</td>
	</tr><tr>
	<td align=right><div class="body_text"> Телефон Number: </td>
	<td align=left><div class="body_text">
	<input type=text size=14 maxlength=12 name=MDPhonENumbeR class="cust_form" value="">&nbsp; (12 digits max - digits only)
	</td>
	</tr><tr>
	<td align=right><div class="body_text"> Поиск Существующих Leads: </td>
	<td align=left><div class="body_text"><input type=checkbox name=LeadLookuP size=1 value="0">&nbsp; (Если эта опция выбрана, то будет предпринята попытка найти телефонный номер в системе, прежде чем вставить его как новый lead)</td>
	</tr><tr>
	...
	<td align=right><div class="body_text"> Отмененный набор номера: </td>
	<td align=left><div class="body_text"><input type=text size=24 maxlength=20 name=MDDiaLOverridE class="cust_form" value="">&nbsp; (только цифры, пожалуйста)
	</td>
	</tr></table>
	<BR>
...
<span style="position:absolute;left:5px;top:<?php echo $CBheight ?>px;z-index:19;" id="VolumeControlSpan"><span id="VolumeUpSpan"><IMG SRC="../agc_ru/images/vdc_volume_up_off.gif" Border=0></span><BR><span id="VolumeDownSpan"><IMG SRC="../agc_ru/images/vdc_volume_down_off.gif" Border=0></span>
</font></span>



<span style="position:absolute;left:35px;top:<?php echo $CBheight ?>px;z-index:20;" id="AgentСтатусSpan"><div class="body_text">
...
<span style="position:absolute;left:157px;top:<?php echo $HTheight ?>px;z-index:32;" id="TransferMain">
	...
	<div class="body_text">
	<IMG SRC="../agc_ru/images/vdc_XB_header.gif" border=0 alt="Перевод - Конференция" style="vertical-align:middle"> &nbsp; &nbsp; &nbsp; &nbsp; <span id="XfeRDiaLGrouPSelecteD"></span> &nbsp; &nbsp; <span id="XfeRCID"></span><BR>
	...
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="LocalCloser"><IMG SRC="../agc_ru/images/vdc_XB_localcloser_OFF.gif" border=0 alt="ЛОКАЛЬНЫЙ CLOSER" style="vertical-align:middle"></span> &nbsp; &nbsp;
	</TD>
	<TD ALIGN=LEFT>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="HangupXferLine"><IMG SRC="../agc_ru/images/vdc_XB_hangupxferline_OFF.gif" border=0 alt="Разъединение Xfer Линии" style="vertical-align:middle"></span>
	</TD>
	</TR>

	<TR>
	<TD ALIGN=LEFT COLSPAN=2>
	<IMG SRC="../agc_ru/images/vdc_XB_seconds.gif" border=0 alt="seconds" style="vertical-align:middle"><input type=text size=2 name=xferlength maxlength=4 class="cust_form">
	&nbsp; 
	<IMG SRC="../agc_ru/images/vdc_XB_channel.gif" border=0 alt="channel" style="vertical-align:middle"><input type=text size=12 name=xferchannel maxlength=200 class="cust_form">
	</TD>
	<TD ALIGN=LEFT>
	<input type=checkbox name=consultativexfer size=1 value="0"><div class="body_tiny"> КОНСУЛЬТАТИВНЫЙ &nbsp;</div>
	</TD>
	<TD ALIGN=LEFT>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="HangupBothLines"><a href="#" onclick="bothcall_send_hangup();return false;"><IMG SRC="../agc_ru/images/vdc_XB_hangupbothlines.gif" border=0 alt="Разъединить Обе Линии" style="vertical-align:middle"></a></span>
	</TD>
	</TR>

	<TR>
	<TD ALIGN=LEFT COLSPAN=2>
	<IMG SRC="../agc_ru/images/vdc_XB_number.gif" border=0 alt="Номер для вызова" style="vertical-align:middle">
	&nbsp; 
	...
	<TD ALIGN=LEFT>
	<input type=checkbox name=xferoverride size=1 value="0"><div class="body_tiny"> НАБЕРИТЕ ОТМЕНУ</div>
	</TD>
	<TD ALIGN=LEFT>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="Leave3WayCall"><a href="#" onclick="leave_3way_call('FIRST');return false;"><IMG SRC="../agc_ru/images/vdc_XB_leave3waycall.gif" border=0 alt="ОСТАВИТЬ 3-WAY ЗАПРОС" style="vertical-align:middle"></a></span> 
	</TD>
	</TR>

	<TR>
	<TD ALIGN=LEFT COLSPAN=4>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="DialBlindTransfer"><IMG SRC="../agc_ru/images/vdc_XB_blindtransfer_OFF.gif" border=0 alt="Слепой Перевод Набора номера" style="vertical-align:middle"></span>
	&nbsp;
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="DialWithCustomer"><a href="#" onclick="SendManualDial('YES');return false;"><IMG SRC="../agc_ru/images/vdc_XB_dialwithcustomer.gif" border=0 alt="Набор номера С Клиентом" style="vertical-align:middle"></a></span> 
	&nbsp;
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="ParkCustomerDial"><a href="#" onclick="xfer_park_dial();return false;"><IMG SRC="../agc_ru/images/vdc_XB_parkcustomerdial.gif" border=0 alt="Парковать Набор номера Клиента" style="vertical-align:middle"></a></span> 
	&nbsp;
	<div class="body_tiny">
	...
	&nbsp;
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="DialBlindVMail"><IMG SRC="../agc_ru/images/vdc_XB_ammessage_OFF.gif" border=0 alt="Blind Transfer VMail Message" style="vertical-align:middle"></span>
	</TD>
	</TR>
...
<div class="body_small"><span style="position:absolute;left:<?php echo $CLwidth ?>px;top:<?php echo $SLheight ?>px;z-index:24;" id="callsinqueuelink">
<?php 
...
<span style="position:absolute;left:60px;top:<?php echo $BPheight ?>px;width:400;height:500;overflow:scroll;z-index:33;background-color:<?php echo $SIDEBAR_COLOR ?>;" id="AgentXferViewSpan"><CENTER><div class="body_text">
...
<div class="body_small"><span style="position:absolute;left:200px;top:<?php echo $CBheight ?>px;z-index:28;" id="dialableleadsspan">
<?php 
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:34;" id="HotKeyActionBox">
	...
	<div class="sd_text"><span id="HotKeyDispo"> - </span></div></CENTER>
	</TD>
	</TR></TABLE>
</span>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:35;" id="HotKeyEntriesBox">
    <table border=0 bgcolor="#FFDD99" width=<?php echo $HCwidth ?> height=70>
	<TR bgcolor="#FFEEBB"><TD width=200><div class="sh_text"> Горячие Клавиши Диспозиции: </div></td><td colspan=2>
	<div class="body_small">Когда активно, просто нажмите клавишу желаемой диспозиции для этого запроса. Тогда запрос будет автоматически разъединен и dispositioned:</div></td></tr><tr>
	<TD width=200><div class="sk_text">
	<span id="HotKeyBoxA"><?php echo $HKboxA ?></span>
	</div></TD>
	<TD width=200><div class="sk_text">
	<span id="HotKeyBoxB"><?php echo $HKboxB ?></span>
	</div></TD>
	<TD><div class="sk_text">
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:36;" id="CBcommentsBox">
	...
	<TR bgcolor="#FFFF66">
	<TD align=left><div class="sh_text"> Предыдущая Информация о Callback: </div></td>
	<TD align=right><div class="sk_text"> <a href="#" onclick="CBcommentsBoxhide();return false;">close</a> </div></td>
	</tr><tr>
	<TD><div class="sk_text">
	...
	<TD width=320><div class="sk_text">
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:37;" id="EAcommentsBox">
    <table border=0 bgcolor="#FFFFCC" width=<?php echo $HCwidth ?> height=70>
	<TR bgcolor="#FFFF66">
	<TD align=left><div class="sh_text"> Расширенная информация Alt Телефон:</div></td>
	<TD align=right><div class="sk_text"> <a href="#" onclick="EAcommentsBoxhide('YES');return false;"> минимизировать </a> </div></td>
	</tr><tr>
	<TD VALIGN=top><div class="sk_text">
	<span id="EAcommentsBoxC"></span><BR>
	<span id="EAcommentsBoxB"></span><BR>
	</div></TD>
	<TD width=320 VALIGN=top><div class="sk_text">
...
<span style="position:absolute;left:695px;top:<?php echo $HTheight ?>px;z-index:38;" id="EAcommentsMinBox">
	...
	<TR bgcolor="#FFFF66">
	<TD align=left><div class="sk_text"><a href="#" onclick="EAcommentsBoxshow();return false;"> максимизировать </a> <BR>Alt Телефон Info</div></td>
	</tr></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:55;" id="NothingBox">
    <BUTTON Type=button name="inert_button"><img src="../agc_ru/images/blank.gif"></BUTTON>
...
<span style="position:absolute;left:<?php echo $HKwidth ?>px;top:<?php echo $HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc_ru/images/vdc_XB_hotkeysactive_OFF.gif" border=0 alt="НЕАКТИВНЫЕ ГОРЯЧИЕ КЛАВИШИ"></a></span>
<?php } ?>
...
<BR><BR>&nbsp;</span></div></td><td align=right height=32>
</td></tr>
<tr><td colspan=3><span id="outboundcallsspan"></span></td></tr>

<tr><td colspan=3><div class="body_small"><span id="AgentAlertSpan">
<?php
...
<span STYLE="background-color: #CCFFCC" id="DiaLControl"><a href="#" onclick="ManualDialNext('','','','','','0');"><IMG SRC="../agc_ru/images/vdc_LB_dialnextnumber_OFF.gif" border=0 alt="Набрать следующий номер"></a></span><BR>
...
<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="RecorDControl"><a href="#" onclick="conf_send_recording('MonitorConf',session_id,'');return false;"><IMG SRC="../agc_ru/images/vdc_LB_startrecording_ru.gif" border=0 alt="Начать Запись"></a></span><BR>
<span id="SpacerSpanA"><IMG SRC="../agc_ru/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: #FFFFFF" id="WebFormSpan"><IMG SRC="../agc_ru/images/vdc_LB_webform_OFF.gif" border=0 alt="Web Форма"></span><BR>
<?php
if ($enable_second_webform > 0)
	{echo "<span STYLE=\"background-color: #FFFFFF\" id=\"WebFormSpanTwo\"><IMG SRC=\"../agc_ru/images/vdc_LB_webform_two_OFF.gif\" border=0 alt=\"Web Форма 2\"></span><BR>\n";}
?>
<span id="SpacerSpanB"><IMG SRC="../agc_ru/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="ParkControl"><IMG SRC="../agc_ru/images/vdc_LB_parkcall_OFF_ru.gif" border=0 alt="Парковать Вызов"></span><BR>
<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="XferControl"><IMG SRC="../agc_ru/images/vdc_LB_transferconf_OFF_ru.gif" border=0 alt="Перевод - Конференция"></span><BR>

<?php
if ($quick_transfer_button_enabled > 0)
	{echo "<span STYLE=\"background-color: $MAIN_COLOR\" id=\"QuickXfer\"><IMG SRC=\"../agc_ru/images/vdc_LB_quickxfer_OFF.gif\" border=0 alt=\"Quick Transfer\"></span><BR>\n";}
?>
...
<span id="SpacerSpanC"><IMG SRC="../agc_ru/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: #FFCCFF" id="HangupControl"><IMG SRC="../agc_ru/images/vdc_LB_hangupcustomer_OFF_ru.gif" border=0 alt="Разъединить Клиента"></span><BR>
<span id="SpacerSpanD"><IMG SRC="../agc_ru/images/blank.gif" width=145 height=16 border=0></span><BR>
<div class="text_input" id="SendDTMFdiv"><span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="SendDTMF"><a href="#" onclick="SendConfDTMF(session_id);return false;"><IMG SRC="../agc_ru/images/vdc_LB_senddtmf.gif" border=0 alt="Послать DTMF" align=bottom></a>  <input type=text size=5 name=conf_dtmf class="cust_form" value="" maxlength=50></div></span><BR>
</center>
</font>
</td>
